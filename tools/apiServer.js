...
server.post("/gliders/", function (req, res, next) {
  const error = validateGlider(req.body);
  if (error) {
    res.status(400).send(error);
  } else {
    req.body.slug = createSlug(req.body.name); // Generate slug for new gliders.
    next();
  }
});
...
function validateGlider(glider) {
  if (!glider.name) return "Name is required.";
  if (!glider.manufacturerId) return "Manufacturer is required.";
  if (!glider.category) return "Category is required.";
  return "";
}
